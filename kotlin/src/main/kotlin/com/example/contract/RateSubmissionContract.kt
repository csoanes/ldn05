 ...
 */
open class RateSubmissionContract() : Contract {
    /**
...
//        data class Amend(): TypeOnlyCommandData, Commands
    ...
    interface Clauses {
        ...
        class Group : GroupClauseVerifier<RateSubmissionState, Commands, UniqueIdentifier>(AnyComposition(Place())) {
            override fun groupStates(tx: TransactionForContract): List<TransactionForContract.InOutGroup<RateSubmissionState, UniqueIdentifier>>
                    // Group by purchase order linearId for in / out states
                    = tx.groupStates(RateSubmissionState::linearId)
        }

        class Place : Clause<RateSubmissionState, Commands, UniqueIdentifier>() {
            override fun verify(tx: TransactionForContract,
                                inputs: List<RateSubmissionState>,
                                outputs: List<RateSubmissionState>,
                                commands: List<AuthenticatedObject<Commands>>,
