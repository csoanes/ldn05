...
fun main(args: Array<String>) {
    RateSubmissionClientRPC().main(args)
}

private class RateSubmissionClientRPC {
    companion object {
        val logger: Logger = loggerFor<RateSubmissionClientRPC>()
    }

    fun main(args: Array<String>) {
        require(args.size == 1) { "Usage: RateSubmissionClientRPC <node address>" }
        ...
        futureTransactions.startWith(transactions).toBlocking().subscribe { transaction ->
            transaction.tx.outputs.forEach { output ->
                val state = output.data as RateSubmissionState
                logger.info(state.rateSubmission.toString())
            }
        }
    }
}
